...
public class PrimaryBloggerActivity extends ClassdroidActivity implements OnDismissListener {
    ...
    private void populate() {
        ...
        editUsername.setText(service.getUsername() == null ? "" : service.getUsername());
        editPassword.setText(service.getPassword() == null ? "" : service.getPassword());
    ...
    private void testAction() {
        ...
        dialog.show();
        btnSave.setEnabled(false);
        thread = new WPLoginCheckThread(editURL.getText().toString().trim(), editUsername.getText()
                .toString().trim(), editPassword.getText().toString().trim(), handler);
        thread.start();
    }
    ...
    private void saveAction() {
        ...
        if (!cbDefault.isChecked()) {
            ...
            utils.setDefaultCredsPrimary(editUsername.getText().toString(), editPassword.getText()
                    .toString());
        } else {
        ...
        if (!cbDefault.isChecked() && isUsernamePasswordOK() || cbDefault.isChecked() && isURLOK()) {
            DBAdapter dbAdapter = new DBAdapter(this);
            dbAdapter.open();
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == 1) {
                if (thread.isOK()) {
                    Toast.makeText(getBaseContext(), getString(R.string.lab_credentials_ok), Toast.LENGTH_LONG).show();
                    enableAllControls();
                } else {
                    Toast.makeText(getBaseContext(), getString(R.string.lab_credentials_wrong), Toast.LENGTH_LONG).show();
                }
            }
